/**
 * Buddy Fleets - Complete India Geo Data & Live Auto-Resolver Engine
 * Supports: Static State/District Mapping + Live India Post & OpenStreetMap APIs
 */
...
export const INDIA_STATES_DISTRICTS = {
  ...
  "Madhya Pradesh": [
    ...
    "Mandsaur", "Morena", "Mauganj", "Narsinghpur", "Neemuch", "Niwari", "Panna", "Pandhurna", 
...
// All Indian States Array
export const INDIAN_STATES = Object.keys(INDIA_STATES_DISTRICTS);

/**
 * Live Dual-Engine Pincode Resolver (India Post + OpenStreetMap Fallback)
 */
export const fetchLocationByPincode = async (pincode) => {
  ...
  try {
    ...
    console.warn("India Post API offline, trying map engine fallback...");
  }

  // Engine 2: OpenStreetMap Live Geocoding Fallback
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 2000);
  ...
  return {
    success: false,
    message: "PIN code not resolved automatically. Please select State & District from dropdown."
  };
};